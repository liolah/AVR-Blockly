...
volatile uint8_t iCtrl[TM1650_NUM_DIGIT];
volatile uint8_t iData[TM1650_NUM_DIGIT];
volatile uint8_t iMode = 0;
...
void TM1650_init() {
  I2C_write_x(TM1650_DISPLAY_BASE, 0x00);
...
void TM1650_sendControl(unsigned char data, uint8_t segment) {
  ...
  I2C_write_x(TM1650_CONTROL_BASE + segment, data);
  _delay_ms(1); //optional
  }

void TM1650_sendDigit(unsigned char data, uint8_t segment) {
  segment = segment % TM1650_NUM_DIGIT;
  I2C_write_x(TM1650_DISPLAY_BASE + segment, data);
  _delay_ms(1); //optional
  }

void TM1650_displayOn() {
  char i;
  for (i = 0; i < TM1650_NUM_DIGIT; i++) {
...
void TM1650_writeNumber(unsigned int num, bool zeroFill) {
